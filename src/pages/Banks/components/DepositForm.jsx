...
export function DepositForm({ open, onOpenChange, onSubmit, isSubmitting, allBanks }) {
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className={`max-w-md ${language === 'bn' ? 'font-bengali' : ''}`}>
                <DialogHeader>
                    <DialogTitle>{t('app.deposit')}</DialogTitle>
                    <DialogDescription>
                        {t({ en: "Select a bank and enter deposit details.", bn: "একটি ব্যাংক নির্বাচন করুন এবং জমার বিস্তারিত লিখুন।" })}
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                                    ...
                                    <Select onValueChange={field.onChange} value={field.value}>
                                        <FormControl>
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder={allBanks?.length > 0 ? t({ en: "Select a bank", bn: "একটি ব্যাংক নির্বাচন করুন" }) : t('app.noBanksAvailable')} />
                                            </SelectTrigger>
                                        </FormControl>
                                        <SelectContent>
                                            {allBanks?.map((bank) => (
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input
                                                ...
                                                value={field.value ? `E${field.value}` : 'E'}
                                                onChange={(e) => {
                                                    const value = e.target.value;
                                                    if (value.startsWith('E')) {
                                                        field.onChange(value.slice(1));
                                                    } else {
                                                        // If user tries to remove E, keep it
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel>{t({ en: "Description", bn: "ডেসক্রিপশন" })}</FormLabel>
                                    <FormControl>
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" disabled={isSubmitting}>
                                {isSubmitting ? t('app.submitting') : t('app.deposit')}
                            </Button>
                        </DialogFooter>
                    </form>
                </Form>
